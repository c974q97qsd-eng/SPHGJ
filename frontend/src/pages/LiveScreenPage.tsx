...
function LiveCard({ it, cardFields, metricDict }: { it: LiveScreenItem; cardFields: string[]; metricDict: MetricDef[] }) {
  ...
  useEffect(() => {
    ...
    try {
      // rev14: 限制 MSE 缓冲,贴着直播边缘并主动修剪旧分片,避免长跑后 WebView2 JS 堆缓涨
      const p = flvjs.createPlayer(
        { type: "flv", url: it.stream_url, isLive: true },
        {
          enableStashBuffer: false,           // 不预攒缓冲(直播无需抗抖动缓存)
          liveBufferLatencyChasing: true,     // 开启延迟追逐:缓冲超阈值即丢旧分片
          liveBufferLatencyMaxLatency: 3.0,   // 最大延迟 3s,超过即修剪,缓冲不长期膨胀
        }
      )
