# -*- coding: utf-8 -*-
"""搜 post/update_sticky_status 的调用参数上下文(纯只读:只下载 JS 静态资源,零 API 调用)。"""
        ...
        for u in urls:
            ...
            for pat in PATS:
                for m in re.finditer(pat, t):
                    a, b = max(0, m.start() - 320), min(len(t), m.end() + 320)
                    found.append({"kw": pat, "u": u[-70:], "ctx": t[a:b]})
        io.open(os.path.join(OUT, "sticky_ctx.json"), "w", encoding="utf-8").write(
